 ...
 */
  ...
  onSubmit(): void {
    ...
    if (this.lockScreenForm.valid) {
      this.authenticationService.login(this.authenticationService.currentUser()?.email!, this.formValues.password?.value,this.authenticationService.currentUser()?.publicationid!)
        .pipe(first())
        .subscribe(
          (data: any) => {
